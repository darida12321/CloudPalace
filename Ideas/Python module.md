...
In older versions, import-related information was spread across `__file__`, `__cached__`, `__loader__`, `__package__`. These were moved to `module.__spec__`, which is a [[Python importlib module module spec|ModuleSpec]] object.

Having a `__path__` attribute turns the **module** into a [[Python package|package]].

**Attributes**:
- `module.__name__`: Unique name. For directly executed modules, it's `"__main__"`.
- `module.__spec__`: The module's import-related [[Python importlib module module spec|module spec]].
